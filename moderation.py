...
class Moderation(commands.Cog):
    ...
    async def warn(self, ctx, membre: discord.Member, *, raison: str = "Aucune raison"):
        ...
        await utils.envoyer_log(
            self.store, ctx.guild, "⚠️ Membre warn", f"Membre : {membre.mention}\nRaison : {raison}\nTotal : {nb} warn(s)",
    ...
    async def unwarn(self, ctx, membre: discord.Member):
        ...
        await utils.envoyer_log(
            self.store, ctx.guild, "✨ Warns effacés", f"Les avertissements de {membre.mention} ont été remis à zéro.",
    ...
    async def warns(self, ctx, membre: discord.Member = None):
        ...
        gid, uid = str(ctx.guild.id), str(membre.id)
        liste = self.store.warns.get(gid, {}).get(uid, [])

        embed = discord.Embed(title=f"⚠️ Avertissements de {membre.display_name}", color=config.Couleurs.AVERTISSEMENT)
        embed.set_thumbnail(url=membre.display_avatar.url)

        if not liste:
            embed.description = "Aucun avertissement."
        else:
            embed.add_field(name="Total", value=str(len(liste)), inline=False)
            for i, w in enumerate(reversed(liste[-10:]), start=1):
                embed.add_field(
                    name=f"#{len(liste) - i + 1} — {w.get('date', 'date inconnue')}",
                    value=f"Raison : {w.get('raison', 'Aucune raison')}\nPar : <@{w.get('by', '?')}>",
                    inline=False,
                )
            if len(liste) > 10:
                embed.set_footer(text=f"Affiche les 10 plus récents sur {len(liste)}.")
    ...
    async def mute(self, ctx, membre: discord.Member, duree_str: str, *, raison: str = "Aucune raison"):
        ...
        await ctx.send(embed=embed)
        await utils.envoyer_log(
            self.store, ctx.guild, "🔇 Membre mute", f"Membre : {membre.mention}\nDurée : {duree_str}\nRaison : {raison}",
    ...
    async def unmute(self, ctx, membre: discord.Member):
        ...
        await utils.envoyer_log(
            self.store, ctx.guild, "🔊 Membre unmute", f"L'exclusion de {membre.mention} a été retirée.",
            config.Couleurs.SUCCES, ctx.author,
        )
    ...
    async def lock(self, ctx, *, raison: str = "Aucune raison"):
        ...
        await ctx.send(embed=embed)
        await utils.envoyer_log(
            self.store, ctx.guild, "🔒 Salon verrouillé", f"Salon : {cible.mention}\nRaison : {raison}",
    ...
    async def unlock(self, ctx):
        ...
        embed.add_field(name="Salon", value=cible.mention, inline=True)
        await ctx.send(embed=embed)
        await utils.envoyer_log(self.store, ctx.guild, "🔓 Salon déverrouillé", f"Salon : {cible.mention}", config.Couleurs.SUCCES, ctx.author)
    ...
    async def kick(self, ctx, membre: discord.Member, *, raison: str = "Aucune raison"):
        ...
        embed = discord.Embed(title="👢 Membre expulsé", color=config.Couleurs.ORANGE_KICK)
        embed.add_field(name="Membre", value=str(membre), inline=True)
        embed.add_field(name="Raison", value=raison, inline=True)
        await ctx.send(embed=embed)
        await utils.envoyer_log(
            self.store, ctx.guild, "👢 Membre kické", f"Pseudo : **{membre}**\nRaison : {raison}",
    ...
    async def ban(self, ctx, membre: discord.Member, *, raison: str = "Aucune raison"):
        ...
        await ctx.send(embed=embed)
        await utils.envoyer_log(
            self.store, ctx.guild, "🔨 Membre banni", f"Pseudo : **{membre}**\nRaison : {raison}",
    ...
    async def unban_cmd(self, interaction: discord.Interaction, user_id: str):
        try:
            ...
            await utils.envoyer_log(
                self.store, interaction.guild, "🔓 Membre débanni", f"L'utilisateur **{user}** ({user_id}) a été débanni.",
