"""Cog "activité" : pause de salon + système d'XP (gardés dans le même
on_message volontairement, voir note dans l'ancien bot : la pause doit
court-circuiter avant tout gain d'XP pour le même message)."""
...
class Activity(commands.Cog):
    ...
    @app_commands.command(name="topmessages", description="Top 5 des membres qui ont envoyé le plus de messages")
    async def topmessages_cmd(self, interaction: discord.Interaction):
        gid = str(interaction.guild.id)
        top = sorted(
            self.store.levels.get(gid, {}).items(),
            key=lambda x: x[1].get("messages", 0), reverse=True,
        )[:5]
        top = [(uid, d) for uid, d in top if d.get("messages", 0) > 0]

        if not top:
            await interaction.response.send_message("Aucun message enregistré pour l'instant.")
            return

        medailles = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
        lignes = [f"{medailles[i]} <@{uid}> — **{d.get('messages', 0)}** message(s)" for i, (uid, d) in enumerate(top)]
        embed = discord.Embed(title="🏆 Top messages", description="\n".join(lignes), color=config.Couleurs.OR)
        await interaction.response.send_message(embed=embed)
