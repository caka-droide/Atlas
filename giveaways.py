...
from cogs.invites import count_new_invites
...
class Giveaways(commands.Cog):
    ...
    def _gw_settings(self, guild_id: int) -> dict:
        ...
        return {
            ...
            "bonus_percent": conf.get("bonus_percent", config.DEFAULT_GIVEAWAY_BONUS_PERCENT),
        }
    ...
    def _weighted_participants(self, data: dict, participants: list) -> list:
        ...
        bonus_percent = data.get("bonus_percent", config.DEFAULT_GIVEAWAY_BONUS_PERCENT)
    ...
    async def gw_cmd(self, interaction: discord.Interaction, duree: str, prix: str, gagnants: int = 1):
        ...
        embed = discord.Embed(title="🎉 GIVEAWAY 🎉", description=description, color=config.Couleurs.JAUNE)
    ...
    @is_admin_staff_or_higher_app()
    async def gwinv_cmd(self, interaction: discord.Interaction, minimum: int,
                         bonus_pourcent: float = config.DEFAULT_GIVEAWAY_BONUS_PERCENT):
        if minimum < 0:
