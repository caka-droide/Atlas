...
class Invites(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        salon_id = guild_config.get(self.store, member.guild.id, "welcome_channel_id")
        salon_bienvenue = member.guild.get_channel(salon_id) if salon_id else None
        if salon_bienvenue:
            embed = discord.Embed(
    ...
    async def on_member_remove(self, member: discord.Member):
        gid = str(member.guild.id)
        uid = str(member.id)
        salon_id = guild_config.get(self.store, member.guild.id, "leave_channel_id")
        salon_depart = member.guild.get_channel(salon_id) if salon_id else None
        if salon_depart:
            embed = discord.Embed(
    ...
    async def i_cmd(self, ctx, *args: str):
        """
        !i                -> stats globales de l'auteur
        !i 5              -> + 5 dernières invitations avec l'heure
        !i @membre        -> stats globales d'un autre membre
        !i @membre 5      -> stats globales d'un autre membre + ses 5 dernières invitations

        (Reste une commande texte uniquement : la syntaxe flexible
        `*args` n'est pas compatible avec les commandes slash. La version
        slash équivalente `/invitations` renvoie vers cette commande.)
        """
        cible = ctx.author
        nombre = None
        ...
        for arg in args:
            ...
            if candidat and not (ctx.message and ctx.message.mentions):
                cible = candidat
            else:
                nombre = valeur
    ...
    async def invitations_cmd(self, interaction: discord.Interaction):
        prefixe = guild_config.get(self.store, interaction.guild.id, "prefix") or config.DEFAULT_PREFIX
        await interaction.response.send_message(
            f"👉 Utilise la commande `{prefixe}i` dans le chat pour voir tes invitations !", ephemeral=True
        )
    ...
    async def topinvites_cmd(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="🏆 Top invitations", description="\n".join(lignes), color=config.Couleurs.DEFAUT)
